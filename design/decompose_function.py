...
if args.mode == 'sigmoid':
    for i,j in enumerate(node):
        if j.name[-7:] == 'Sigmoid':
            print("######%s######" % i)
            old_node = j
            onnx_model.graph.node.remove(j)
            ...
            graph.initializer.insert(0, initializer_new)
            ...
            graph.node.insert(i, new_scale_node_1)
...
if args.mode == 'softmax':
    ...
    for i,j in enumerate(node):
        if j.name == "/LogSoftmax":
            print("######%s######" % i)
            old_node = j
            print('======update ', old_node.name, '=========')
            onnx_model.graph.node.remove(j)
            axes1 = onnx.numpy_helper.from_array(select_axes, "logprobs_reducesum_axes")
            new_scale_node_2 = onnx.helper.make_node(
                op_type="ReduceMax",
                inputs=old_node.input,
            ...
            graph.node.insert(i, new_scale_node_1)
            all_initializer.insert(0, axes1)
            graph.node.insert(i + 1, new_scale_node_2)
            graph.node.insert(i + 2, new_scale_node_3)
            graph.node.insert(i + 3, new_scale_node_4)
    ...
    for i, j in enumerate(node):
        if j.name[-8:] == "/Softmax":
            ...
            axes1 = onnx.numpy_helper.from_array(select_axes2, initia_name)
...
if args.mode=='exp':
    ...
    for i,j in enumerate(node):
        if j.name[-3:] == 'Exp':
            ...
            value1 = onnx.numpy_helper.from_array(ln_2_value, ln_2_name)
            ...
            all_initializer.insert(0, value1)
            all_initializer.insert(0, value2)
            graph.node.insert(i, new_scale_node_1)
            graph.node.insert(i + 1, new_scale_node_2)


onnx.save(onnx_model, args.model_path_out)
